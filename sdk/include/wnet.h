...
#define WNET_SIM_INVALID			(-4999)	// sim card invalid
...
typedef struct {
	char		Name[128];			//模块收单机构名称
	int         Number;             //客户编号,1000-9999
} wnet_acquirerInfo_t;
...
/*other */
	...
	int		(*wnet_GetAcqInfo)(int, wnet_acquirerInfo_t *);  //获取模块的收单信息
	int		(*wnet_SetAcqInfo)(int, const wnet_acquirerInfo_t *);  //设置模块的收单信息
	int		(*wnet_GetVerSimFlag)(int, int *, int *);  //获取验证SIM卡的标识
	int		(*wnet_SetVerSimFlag)(int, int);  //设置验证SIM卡的标识
